...
export default async function getMovies(id: any, type: any) {
  console.log("this is the movie type", type);
  const axiosConfig = {
    ...header,
    headers: {
      ...header.headers,
  ...
  var {
    data: { results },
  } = await axios.get(movie_videos_api, axiosConfig);
  var {
    data: { backdrops, posters },
  } = await axios.get(movie_images_api, axiosConfig);
  const {
    data: { logos: logos },
  } = await axios.get(movie_logo_api, axiosConfig);
  var logo = logos.filter(
    (item: { iso_639_1: string }) => item.iso_639_1 == "en"
  )[0]?.file_path;
  if (!logo) {
    logo = logos[0]?.file_path;
    if (!logo) logo = null;
  }
  return { data, logo, results, backdrops, posters };
}
